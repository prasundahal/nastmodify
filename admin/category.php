...
$parentId = $_GET['parentId'];
	...
	if(isset($_POST['save']) && $_POST['save'] == "Save")
	{
		...
		if (empty($msg)) {
			$catId = $groups->saveCategory('', $name, $urlname, $parentId, $content, $weight, $pageTitle, $pageKeyword);
			...
			header("Location: category.php?parentId=".$_GET['parentId']."&msg=$msg");
			exit();
		}
		else {
			header("Location: category.php?parentId=".$_GET['parentId']."&msg=$msg");
	...
	if(isset($_POST['edit']) && $_POST['edit'] == "Edit")
	{
		...
		if (empty($msg)) {
			$groups->saveCategory($_GET['eid'], $name, $urlname, $parentId, $content, $weight, $pageTitle, $pageKeyword);
			
			$groups->saveImage($_GET['eid']);
			
			$msg="Category Updated Successfully";
			
			header("Location: category.php?action=edit&parentId=".$_GET['parentId']."&eid=".$_GET['eid']."&msg=$msg");
			exit();
		}
		else {
			header("Location: category.php?action=edit&parentId=".$_GET['parentId']."&eid=".$_GET['eid']."&msg=$msg");
...
if (isset($_GET['deleteImage'])) {
	...
	header ("Location: category.php?action=edit&parentId=".$_GET['parentId']."&eid=".$_GET['id']."&msg=$msg");
}
?>
<?
...
function createDrpMenu1($catId, $startDash)
{
	...
	{
	?>
    <option value="category.php?parentId=<?= $catRow['id']?>" <? if($_GET['parentId'] == $catRow['id']){?> selected<? }?>>
    <?= $dash.$catRow['name'];?>
    </option>
...
<form action="<?= $_REQUEST['uri'];?>" method="post" onsubmit="return frmvalidate(this)" enctype="multipart/form-data">
        <table width="100%" border="0" cellspacing="2" cellpadding="0">
          <tr>
            ...
            <a href="category.php" style="float:right; padding-right:10px; text-decoration:none; color:#cc0000;">Add New</a></th>
          </tr>
          <tr>
...
?>
              ...
              <table width="100%" border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <th colspan="2" class="heading2">Manage Category <a href="category.php" style="float:right; padding-right:10px; text-decoration:none; color:#cc0000;">Add New</a></th>
                </tr>
                <tr>
    ...
    <?
			if($parentId == 0)
			...
			else
				{
					...
					echo "<a href=\"category.php?parentId=".$selRow['parentId']."\" style=\"float:right; color:#cc0000; text-decoration:none; padding-right:10px;\">UP</a>";
				}
		?>
    </th>
    </tr>
  <tr bgcolor="#E7E7E7" class="tahomabold11">
   ...
   <?
	 $counter = 0;
		$catResult = $groups->getByParentIdAndLinkType($parentId, "ProductCategory");
			while($catRow = $conn->fetchArray($catResult))
				{
					$counter++;
	?>
  <tr align="center" <?php if ($counter % 2 == 0) { echo "bgcolor='#F1F1F1'";} ?>>
    ...
    <td>
    	<?
				$prodCountResult = $groups->getByParentId($catRow['id']);
				$prodCount = $conn->numRows($prodCountResult);
				echo $prodCount;
			?>
    </td>
    <td><?=$catRow['weight'];?>
    <td><a href="category.php?parentId=<?= $catRow['id'];?>">Open</a> | 
		<? 	$categoryResult = $groups->getByParentId($catRow['id']);
				$count1 = $conn->numRows($categoryResult);
				$prodResult = $groups->getByParentId($catRow['id']); 
				$count = $conn->numRows($prodResult); if($count < 1 && $count1 < 1) {?><a href="category.php?action=delete&did=<?= $catRow['id']?>&parentId=<?= $parentId?>">Delete</a> |<? }?> <a href="category.php?action=edit&eid=<?= $catRow['id'];?>&parentId=<?= $catRow['parentId']?>">Edit</a></td>
  </tr>
	<?
		}
	?>
